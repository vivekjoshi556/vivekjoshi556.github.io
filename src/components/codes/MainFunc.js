...
const MainFunc = () => {
	...
	const execute = async () => {
		...
		await sendMessage("success", "2023: Finish Master's Thesis.", 1000);
		await sendMessage("success", "First OSS Contribution.", 1000);
		await sendMessage("success", "Employed at EXL Services", 1000, false);
	};
	...
	return (
		<motion.div
			...
			initial="initial"
			animate="animate"
			className="text-gray-200 text-sm code aboveNoise"
		>
			<div className="overflow-x-auto codeScrollbar overflow-y-hidden">
				<motion.div className="w-max" variants={codeVariant}>
				...
				<motion.div className="w-max" variants={codeVariant}>
					...
					*vivek_joshi = new <span className="class">Person</span>
					("03-06-1999");
				</motion.div>
				<motion.div className="w-max" variants={codeVariant}>
			...
			<div className="flex justify-end">
				<button
					...
					className="bg-green-600 text-white hover:bg-green-700 duration-200 py-1 mt-1 py-2 px-3 ml-auto rounded mb-2"
				>
					Clear Console
				</button>
				{!exec.executing && (
					<button
						onClick={execute}
						className="bg-green-600 text-white hover:bg-green-700 duration-200 py-1 mt-1 px-3 ml-1 rounded mb-2 aboveNoise"
					>
						Execute
					</button>
				)}
				{exec.executing && (
					<button className="bg-green-600 text-white hover:bg-green-600 opacity-60 py-1 mt-1 px-3 ml-1 rounded mb-2 cursor-not-allowed aboveNoise">
						<FontAwesomeIcon
							className="fa-spin w-4 dark:text-gray-300"
							icon={faGear}
						></FontAwesomeIcon>{" "}
						Executing
					</button>
