...
const SkillsIndex = () => {
	...
	const sendMessage = async (type, msg, waitFor = 1000, executing = true) => {
		...
		if (localConsole) localConsole.scrollTop = localConsole.scrollHeight;
	};
	...
	return (
		<motion.div
			key="aboutIndex"
			className="min-h-screen w-11/12 mx-auto flex items-center"
			...
			<div className="w-full dark:bg-gray-800 h-full items-center flex justify-center gap-4 flex-wrap md:flex-nowrap">
				<div className="lg:w-8/12 md:w-7/12 w-full flex items-center flex-wrap gap-x-4">
					<div className="w-full mb-4 pt-24 md:pt-0">
						<h2
							className="text-3xl font-bold glitch silkscreen text-gray-200 inline-block"
							data-text="Skills."
						>
							Skills.
						</h2>
					...
					<motion.div
						...
						className="w-full flex items-center flex-wrap gap-x-4"
					>
						{skills.map((skill, i) => (
							<SkillBox
								...
								key={'skill_' + i}
								delay={i * 0.04}
								onClick={() => loadSkill(skill.loader)}
								src={skill.src}
								name={skill.name}
							/>
						))}
				...
				<div className="lg:w-8/12 md:w-5/12 w-full">
					...
					<div className="flex justify-end">
						<button
							...
							className="bg-green-600 text-white hover:bg-green-700 duration-200 mt-1 py-2 px-3 ml-auto rounded mb-2"
						>
							Clear Console
						</button>
					</div>
